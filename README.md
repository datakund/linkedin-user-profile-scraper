...
A python library that uses browser automation to login scrape user profile data from linkedin
...
```sh
pip install linkedin-user-profile-scraper
```

### Import 

```javascript
from linkedin-user-profile-scraper import *
```

### Login

To scrape user profile details on linkedin first we will login to linkedin. There are two ways of login:-
* Credentials
* Cookies
...
To fetch profile details from linkedin profile of user we use ``get_profile`` function.
It requires **profile_link** as input parameter.

```javascript
response=linkedin.get_profile(profile_link="user_profile_link")
profile_details=response['body']
```
